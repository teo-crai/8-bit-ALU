module divider(
  ...
  register#(.w(8)) reg_A(.clk(clk), .load(ld_en), .rst(rst), .d(A_in), .q(A));
  register#(.w(8)) reg_Q(.clk(clk), .load(ld_en), .rst(rst), .d(Q_in), .q(Q));
  register#(.w(8)) reg_M(.clk(clk), .load(start), .rst(rst), .d(divisor), .q(M)); //M se incarca doar la inceput pentru ca nu i se schimba valoarea
  ...
  register#(.w(8)) res_latch_q (.clk(clk), .load(done | div_zero_err), .d(Q_in), .q(quotient), .rst(rst));
  register#(.w(8)) res_latch_r (.clk(clk), .load(done | div_zero_err), .d(A_in), .q(remainder), .rst(rst));
